...
mainGameState.create = function () {
    ...
    mainGameState.objectGroup = game.add.group();

    mainGameState.Matches = 0;
...
mainGameState.addNewPlayer = function (id, x, y) {
    if (id == 1 || id == 2) {
        ...
        mainGameState.objectGroup.add(mainGameState.playerList[id]);


        mainGameState.objectGroup.create(1500, 400, 'redSquareObject');
        mainGameState.scaleObjects(mainGameState.objectGroup.children[mainGameState.objectGroup.children.length - 1]);
        mainGameState.objectGroup.create(1700, 1900, 'blueSquareObject');
        mainGameState.scaleObjects(mainGameState.objectGroup.children[mainGameState.objectGroup.children.length - 1]);
        mainGameState.objectGroup.create(500, 300, 'redObject');
        mainGameState.scaleObjects(mainGameState.objectGroup.children[mainGameState.objectGroup.children.length - 1]);
        mainGameState.objectGroup.create(300, 2100, 'circleObject');
        mainGameState.scaleObjects(mainGameState.objectGroup.children[mainGameState.objectGroup.children.length - 1]);
        mainGameState.objectGroup.create(2200, 1900, 'poleObject');
        mainGameState.scaleObjects(mainGameState.objectGroup.children[mainGameState.objectGroup.children.length - 1]);
        mainGameState.objectGroup.create(1100, 800, 'andObject');
        mainGameState.scaleObjects(mainGameState.objectGroup.children[mainGameState.objectGroup.children.length - 1]);
        mainGameState.objectGroup.create(2200, 700, 'noObject');
        mainGameState.scaleObjects(mainGameState.objectGroup.children[mainGameState.objectGroup.children.length - 1]);
        mainGameState.objectGroup.create(1000, 1300, 'billboardObject');
        mainGameState.scaleObjects(mainGameState.objectGroup.children[mainGameState.objectGroup.children.length - 1]);
        mainGameState.objectGroup.create(1400, 1000, 'followObject');
        mainGameState.scaleObjects(mainGameState.objectGroup.children[mainGameState.objectGroup.children.length - 1]);

        //PROPERLY LAYERS PLAYERS ON THE Z AXIS
        if (!mainGameState.playerList[2]) {
            game.world.swap(mainGameState.objectGroup, mainGameState.dictionary);
            game.world.swap(mainGameState.objectGroup, mainGameState.symbols);
            game.world.swap(mainGameState.dictionary, mainGameState.symbols);
        }

        if (mainGameState.playerList[2] && myID == 2) {
            game.world.swap(mainGameState.objectGroup, mainGameState.dictionary);
            game.world.swap(mainGameState.objectGroup, mainGameState.symbols);
            game.world.swap(mainGameState.dictionary, mainGameState.symbols);
        }
...
mainGameState.scaleObjects = function (object) {
    object.anchor.setTo(.5, .5);
    object.scale.setTo(4, 4);
}
...
mainGameState.update = function () {
    ...
    if (mainGameState.objectGroup) {
        mainGameState.objectGroup.sort('y', Phaser.Group.SORT_ASCENDING);
    }
            ...
            for (var i = 0; i <= mainGameState.activePage.children.length - 1; i++) {
                ...
                if (touchedbutton == mainGameState.activePage.children[i].children[1]) {
                    ...
                    if (touchedbutton.children[0].visible) {
                        ...
                        for (var i = 0; i <= mainGameState.activePage.children.length - 1; i++) {
                            if (mainGameState.activePage.children[i].children[1].visible && mainGameState.activePage.children[i].children[1].children[0].visible) {
                                checkOnPage = true;
                            }
...
mainGameState.match = function (word, phrase, page) {
    ...
    for (var i = 0; i <= active.children.length - 1; i++) {
        if (active.children[i].children[1].visible && active.children[i].children[1].children[0].visible) {
            checkOnPage = true;
        }
        if (!active.children[i].children[1].children[0].visible && !checkOnPage) {
            checkOnPage = false;
        }
    }
